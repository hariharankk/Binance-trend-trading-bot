...
def get_daily_close(data):
    ...
    for coin in data:
        time = (datetime.datetime.fromtimestamp(data[coin]['timestamp'] / 1e3)).strftime('%d %b, %Y')
        daily_close[coin] = get_data(data[coin]['symbol'], time)
        daily_close[coin] = [[item[4] for item in daily_close[coin]], [dt.fromtimestamp(item[0]/1000) for item in daily_close[coin]]]
...
def get_buy_price(data):
    buy_price = {}
    for coin in data:
        buy_price[coin] = data[coin]['bought_at']
...
def main():
    ...
    trades = load_trades('coins_bought.json')
