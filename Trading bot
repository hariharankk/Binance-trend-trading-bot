...
def buy(Crypto,y,t):
    ...
    for Crypto in Crypto:
            ...
            if str(df['Close'].iloc[-1]) > str(df['upper_bound'].iloc[-1]) and str(df['pos'].iloc[-1]) == 1:
...
def sell(coins_bought):
    ...
    if len(coins_bought) != 0:    
        for buy in coins_bought:
            ...
            if str(df['Close'].iloc[-1]) < str(df['lower_bound'].iloc[-1]) or str(df['Close'].iloc[-1]) < str(stoploss) :
                try:
...
if __name__ == '__main__':
    ...
    p1 = Thread(target=buy,args=(Crypto,y,t,))
